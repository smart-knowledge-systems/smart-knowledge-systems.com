...
export default function PortfolioMarkdown({ content, className = "" }: PortfolioMarkdownProps) {
  return (
    <div className={className}>
      <ReactMarkdown 
        ...
        components={{
          ...
          h3: ({ children }) => <h3 className="text-xl font-semibold text-gray-900">{children}</h3>,
          p: ({ children }) => <p className="text-gray-600 leading-relaxed">{children}</p>,
          em: ({ children }) => <em className="italic">{children}</em>,
          strong: ({ children }) => <strong className="font-semibold">{children}</strong>,
          code: ({ children }) => <code className="bg-gray-100 px-1 py-0.5 rounded text-sm font-mono">{children}</code>,
          a: ({ href, children }) => (
            <a href={href} className="text-indigo-600 hover:text-indigo-500 underline" target="_blank" rel="noopener noreferrer">
              {children}
            </a>
          ),
        }}
